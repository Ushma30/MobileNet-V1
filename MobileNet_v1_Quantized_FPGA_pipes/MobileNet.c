...
cl_command_queue commands_convStandard;     // compute command convStandard
cl_command_queue commands_convDepthwise;    // compute command convDepthwise
cl_command_queue commands_convPointwise;    // compute command convPointwise
cl_command_queue commands_avgPool;    // compute command convPointwise
...
cl_event myevent_convStandard; //profiling event
cl_event myevent_convDepthwise; //profiling event
cl_event myevent_convPointwise; //profiling event
cl_event myevent_avgPool; //profiling event
...
int openClCreateContext() {
	...
	// Create a command commands_convStandard
	commands_convStandard = clCreateCommandQueue(context, device_id, CL_QUEUE_PROFILING_ENABLE, &err);
	if (!commands_convStandard)
	{
		printf("Error: Failed to create a command commands!\n");
		return EXIT_FAILURE;
	}

    // Create a command commands_convDepthwise
	commands_convDepthwise = clCreateCommandQueue(context, device_id, CL_QUEUE_PROFILING_ENABLE, &err);
	if (!commands_convDepthwise)
	{
		printf("Error: Failed to create a command commands!\n");
		return EXIT_FAILURE;
	}

    // Create a command commands_convPointwise
	commands_convPointwise = clCreateCommandQueue(context, device_id, CL_QUEUE_PROFILING_ENABLE, &err);
	if (!commands_convPointwise)
	{
		printf("Error: Failed to create a command commands!\n");
		return EXIT_FAILURE;
	}

    // Create a command commands_avgPool
	commands_avgPool = clCreateCommandQueue(context, device_id, CL_QUEUE_PROFILING_ENABLE, &err);
	if (!commands_avgPool)
	{
		printf("Error: Failed to create a command commands!\n");
		return EXIT_FAILURE;
...
int openClCreateKernel() {
    ...
    //FPGA-Change    
    lFileSize = LoadOpenCLKernel("./xclbin/vector_addition.sw_emu.xilinx_aws-vu9p-f1-04261818_dynamic_5_0.xclbin", &KernelSource);
	if( lFileSize < 0L ) {
...
void convStandard (unsigned char* opfm) {
	...
	err = clEnqueueWriteBuffer(commands_convStandard, d_image_r, CL_TRUE, 0, HEIGHT_0*WIDTH_0*sizeof(unsigned char), image_r, 0, NULL, NULL);
	err |= clEnqueueWriteBuffer(commands_convStandard, d_image_g, CL_TRUE, 0, HEIGHT_0*WIDTH_0*sizeof(unsigned char), image_g, 0, NULL, NULL);   
	err |= clEnqueueWriteBuffer(commands_convStandard, d_image_b, CL_TRUE, 0, HEIGHT_0*WIDTH_0*sizeof(unsigned char), image_b, 0, NULL, NULL);   
	err |= clEnqueueWriteBuffer(commands_convStandard, d_filter, CL_TRUE, 0, IP_FM_1*FDIM*FDIM*FDIM*sizeof(unsigned char), filter_proper, 0, NULL, NULL);   
	err |= clEnqueueWriteBuffer(commands_convStandard, d_bias, CL_TRUE, 0, IP_FM_1*sizeof(int), h_bias, 0, NULL, NULL);   
	...
	err = clEnqueueNDRangeKernel(commands_convStandard, standard_conv, 2, NULL,globalWorkSize, localWorkSize, 0, NULL, &myevent_convStandard);   
	...
	//clWaitForEvents(1,&myevent_convStandard);	 
	//clFinish(commands);   
...
void convDepthwise(unsigned char* ipfm, unsigned char* opfm, char* fileName_bias, 
	...
	err = clEnqueueWriteBuffer(commands_convDepthwise, d_input, CL_TRUE, 0, iph*ipw*ip_fsize*sizeof(unsigned char), ipfm, 0, NULL, NULL);
	err |= clEnqueueWriteBuffer(commands_convDepthwise, d_filter, CL_TRUE, 0, op_fsize*FDIM*FDIM*sizeof(unsigned char), filter_proper, 0, NULL, NULL);
	err |= clEnqueueWriteBuffer(commands_convDepthwise, d_bias, CL_TRUE, 0, op_fsize*sizeof(int), h_bias, 0, NULL, NULL);   
	...
	err = clEnqueueNDRangeKernel(commands_convDepthwise, depthwise_conv, 2, NULL, globalWorkSize, localWorkSize, 0, NULL, &myevent_convDepthwise);   
	...
	//clWaitForEvents(1,&myevent_convDepthwise);	 
	//clFinish(commands);   
	//clGetEventProfilingInfo(myevent,CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
	//clGetEventProfilingInfo(myevent,CL_PROFILING_COMMAND_END,sizeof(cl_ulong), &end, NULL);
	//kernelExecTimeNs += end - start;	
/*    if (layer_count == 3)*/
/*    {*/
/*        clFinish(commands_convDepthwise);*/
/*	    err = clEnqueueReadBuffer(commands_convDepthwise, d_output, CL_TRUE, 0, op_fsize*oph*opw*sizeof(unsigned char), opfm, 0, NULL, NULL);*/
/*    }*/
	...
	//arrangOutput(opfm, output_proper, op_fsize, oph*opw);
	//FILE *write_ptr;
...
void convPointwise(unsigned char* ipfm, unsigned char* opfm, char* fileName_bias, 
	...
	err = clEnqueueWriteBuffer(commands_convPointwise, d_input, CL_TRUE, 0, iph*ipw*ip_fsize*sizeof(unsigned char), ipfm, 0, NULL, NULL);
	err |= clEnqueueWriteBuffer(commands_convPointwise, d_filter, CL_TRUE, 0, ip_fsize*op_fsize*FDIM_P*FDIM_P*sizeof(unsigned char), filter, 0, NULL, NULL);
	err |= clEnqueueWriteBuffer(commands_convPointwise, d_bias, CL_TRUE, 0, op_fsize*sizeof(int), h_bias, 0, NULL, NULL);
	...
	globalWorkSize[0] = opw;
	globalWorkSize[1] = oph;
	err = clEnqueueNDRangeKernel(commands_convPointwise, pointwise_conv, 2, NULL, globalWorkSize, localWorkSize, 0, NULL, &myevent_convPointwise);   
    ...
    //clWaitForEvents(1,&myevent_convStandard);
    //clWaitForEvents(1,&myevent_convDepthwise);
	//clWaitForEvents(1,&myevent_convPointwise);	 
	//clFinish(commands_convStandard);
	//clFinish(commands_convDepthwise);
   	//clFinish(commands_convPointwise);   
    

/*   	clGetEventProfilingInfo(myevent_convStandard,CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);*/
/*	clGetEventProfilingInfo(myevent_convStandard,CL_PROFILING_COMMAND_END,sizeof(cl_ulong), &end, NULL);*/
/*	kernelExecTimeNs += end - start;*/
/*    printf("Kernel Execution time for Standard Convolution : %f\n", layer_count, kernelExecTimeNs/1000000000);*/


/*   	clGetEventProfilingInfo(myevent_convDepthwise,CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);*/
/*	clGetEventProfilingInfo(myevent_convDepthwise,CL_PROFILING_COMMAND_END,sizeof(cl_ulong), &end, NULL);*/
/*	kernelExecTimeNs += end - start;*/
/*    printf("Kernel Execution time for Layer Depthwise Convolution: %f\n", layer_count, kernelExecTimeNs/1000000000);*/


/*   	clGetEventProfilingInfo(myevent_convPointwise,CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);*/
/*	clGetEventProfilingInfo(myevent_convPointwise,CL_PROFILING_COMMAND_END,sizeof(cl_ulong), &end, NULL);*/
/*	kernelExecTimeNs += end - start;*/
/*    printf("Kernel Execution time for Layer Pointwise Convolution: %f\n", layer_count, kernelExecTimeNs/1000000000);*/

/*	err = clEnqueueReadBuffer(commands_convPointwise, d_output, CL_TRUE, 0, op_fsize*oph*opw*sizeof(unsigned char), opfm, 0, NULL, NULL);*/
...
void convAvgPool(unsigned char* ipfm, unsigned char* opfm,
	...
	err = clEnqueueWriteBuffer(commands_avgPool, d_input, CL_TRUE, 0, iph*ipw*ip_fsize*sizeof(unsigned char), ipfm, 0, NULL, NULL);
	...
	err = clEnqueueNDRangeKernel(commands_avgPool, avgPool, 1, NULL, globalWorkSize, localWorkSize, 0, NULL, &myevent_avgPool);   
...
/*	clWaitForEvents(1,&myevent);	 */
	clFinish(commands_avgPool);   
/*	clGetEventProfilingInfo(myevent,CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);*/
/*	clGetEventProfilingInfo(myevent,CL_PROFILING_COMMAND_END,sizeof(cl_ulong), &end, NULL);*/
	kernelExecTimeNs += end - start;
	err = clEnqueueReadBuffer(commands_avgPool, d_output, CL_TRUE, 0, op_fsize*oph*opw*sizeof(unsigned char), opfm, 0, NULL, NULL);
	...
	//Get kernel execution time
	printf("Avg pool Kernel Execution time for Layer %d: %f\n", layer_count, kernelExecTimeNs/1000000000);
...
/*	} */
	...
	clReleaseCommandQueue(commands_convStandard);
    clReleaseCommandQueue(commands_convDepthwise);
    clReleaseCommandQueue(commands_convPointwise);
